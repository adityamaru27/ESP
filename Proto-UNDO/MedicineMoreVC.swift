//
//  MedicineMoreVC.swift
//  Proto-UNDO
//
//  Created by Santu C on 16/11/15.
//  Copyright © 2015 Curly Brackets. All rights reserved.
//

import UIKit
enum kMedicineMoreContentCell : Int {
...
class MedicineMoreVC: UITableViewController ,  UIPickerViewDelegate, UIPickerViewDataSource, UIImagePickerControllerDelegate, UINavigationControllerDelegate, UITextFieldDelegate  {
    ...
    override func viewDidLoad() {
      ...
      if ( settingData.Quantity == 0 )
      {
        ...
        lblQuantityResult.textColor = UIColor(red:154/255, green:154/255, blue:154/255, alpha:1.0)
        
        }
        else
      {
        lblQuantityResult.text = "\(settingData.Quantity)"
        ...
        datePicker.addTarget(self, action: #selector(MedicineMoreVC.datePickerValueChanged(_:)), forControlEvents: UIControlEvents.ValueChanged)
        ...
        breastTextInput.addTarget(self, action: #selector(MedicineMoreVC.textFieldDidChange(_:)), forControlEvents: UIControlEvents.EditingChanged)
    ...
    @IBAction func OnTappedDurationStart(sender: UIButton) {
        ...
        if sender.tag == 0
        {
            ...
            if !timer.valid {
                let aSelector : Selector = #selector(MedicineMoreVC.updateTime)
    ...
    @IBAction func onTapSave(sender: AnyObject) {
        ...
        DataSource.sharedDataSouce.logEvent(self.logEvent, block: { (success, error) -> Void in
            ...
            if (success){
                // self.changed = false
                  NSNotificationCenter.defaultCenter().postNotification(NSNotification(name: kLogObjectAddingNotification, object: self.logEvent))
    ...
    func refreshViews() {
        if !settingData.bIsNotJustNow {
            ...
            lblDuration1.text = "Duration \(logEvent.item!.actions[0].name.capitalizedString)"
            lblDuration2.text = "Duration \(logEvent.item!.actions[0].name.capitalizedString)"
            ...
            lblTimeBothText.text = "Time \(logEvent.item!.actions[0].name.capitalizedString)"
            lblDurationJust.text = "Duration \(logEvent.item!.actions[0].name.capitalizedString)"
            lblDurationJustExpand.text = "Duration \(logEvent.item!.actions[0].name.capitalizedString)"
    ...
    func imagePickerController(picker: UIImagePickerController, didFinishPickingMediaWithInfo info: [String : AnyObject]){
        ...
        let tapImage = UITapGestureRecognizer(target: self, action: #selector(MedicineMoreVC.imageTapped(_:)))
    ...
    override func tableView(tableView: UITableView, heightForRowAtIndexPath indexPath: NSIndexPath) -> CGFloat {
        ...
        let currCell:kMedicineMoreContentCell = kMedicineMoreContentCell(rawValue: indexPath.row)!
    ...
    func pickerView(pickerView: UIPickerView, didSelectRow row: Int, inComponent component: Int) {
        ...
        if pickerView == self.pickerQuanityResult {
            ...
            {
                lblQuantityResult.text = "Not Set"
                lblQuantityResult.textColor = UIColor(red:239/255, green:239/255, blue:244/255, alpha:1.0)
