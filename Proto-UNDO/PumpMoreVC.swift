...
class PumpMoreVC: UITableViewController ,  UIPickerViewDelegate, UIPickerViewDataSource, UIImagePickerControllerDelegate, UINavigationControllerDelegate, UITextFieldDelegate  {
    ...
    var settingData:PumpMoreSettingData = PumpMoreSettingData()
    ...
    var optAction:PumpMoreSettingData.PumpActions! = .ActLeft
  
    
    
    var bItemChanged:Bool = false
    var init_settingData:PumpMoreSettingData = PumpMoreSettingData()
 ...
 //   @IBOutlet weak var sliderQuantityBottle: UISlider!
    ...
    override func viewDidLoad() {
        ...
        {
            initTime = settingData.timeRight
        }
        //settingData.timeRight = initTime
        
        datePicker.addTarget(self, action: #selector(PumpMoreVC.datePickerValueChanged(_:)), forControlEvents: UIControlEvents.ValueChanged)
        ...
        breastTextInput.addTarget(self, action: #selector(PumpMoreVC.textFieldDidChange(_:)), forControlEvents: UIControlEvents.EditingChanged)
        ...
        btnSave.enabled = false
          ...
          addtextCell(8, textValue: "Stopwatch")
          addtextCell(12, textValue: "Timer")
    ...
    @IBAction func OnChangedNotJustNow(sender: UISwitch) {
        ...
        if !sender.on {
            if settingData.optAction == PumpMoreSettingData.PumpActions.ActLeft {
                settingData.timeLeft = initTime
            }
            else if settingData.optAction == PumpMoreSettingData.PumpActions.ActBoth {
                settingData.timeBoth = initTime
            }
            else
            {
                settingData.timeRight = initTime
    ...
    @IBAction func OnTappedDurationStart(sender: UIButton) {
        ...
        if sender.tag == 0
        {
            ...
            if !timer.valid {
                let aSelector : Selector = #selector(PumpMoreVC.updateTime)
    ...
    @IBAction func onTapSave(sender: AnyObject) {
       ...
       else  if settingData.optAction == PumpMoreSettingData.PumpActions.ActBoth
        {
            ...
            logEvent.bothHours = settingData.nLeftDurationHours
            logEvent.bothMins = settingData.nLeftDurationMinutes
        }
        else
        {
            logEvent.rightHours = settingData.nLeftDurationHours
            logEvent.rightMins = settingData.nLeftDurationMinutes
...
//        logEvent.value = Float(self.txtQty.text!)!
    ...
    func datePickerValueChanged(sender: UIDatePicker)
    {
        ...
        if settingData.optAction == PumpMoreSettingData.PumpActions.ActLeft  {
            ...
            settingData.timeBoth = datePicker.date
        }
        else
        {
    ...
    @IBAction func segmnetControlValueChange(sender: AnyObject) {
    
        switch self.segmentAction.selectedSegmentIndex {
        case 0:
                settingData.optAction = PumpMoreSettingData.PumpActions.ActBoth
                logEvent.action = "both"
        case 1:
                settingData.optAction = PumpMoreSettingData.PumpActions.ActLeft
                logEvent.action = "left"
        case 2:
                settingData.optAction = PumpMoreSettingData.PumpActions.ActRight
        ...
        self.optAction = settingData.optAction
        initBottleUIs()
    ...
    func refreshViews() {
        if !settingData.bIsNotJustNow {
            if settingData.optAction == PumpMoreSettingData.PumpActions.ActLeft  {
                lblTimeBothText.text = "Time Pump Left"
                lblDuration1.text = "Duration Pump Left"
                lblDuration2.text = "Duration Pump Left"
                lblDurationInfo1.text = ""
                lblDurationInfo2.text = ""
            }
            else if settingData.optAction == PumpMoreSettingData.PumpActions.ActRight
            {
                lblTimeBothText.text = "Time Pump Right"
                lblDuration1.text = "Duration Pump Right"
                lblDuration2.text = "Duration Pump Right"
                lblDurationInfo1.text = ""
                lblDurationInfo2.text = ""
            }
            else
            {
                lblTimeBothText.text = "Time Pump Both"
                lblDuration1.text = "Duration Pump Both"
                lblDuration2.text = "Duration Pump Both"
                lblDurationInfo1.text = ""
                lblDurationInfo2.text = ""
            }
        }
            ...
            if settingData.optAction == PumpMoreSettingData.PumpActions.ActLeft  {
                lblTimeBothText.text = "Time Pump Left"
                lblDurationJust.text = "Duration Pump Left"
                lblDurationJustExpand.text = "Duration Pump Left"
            }
            else if settingData.optAction == PumpMoreSettingData.PumpActions.ActRight
            {
                lblTimeBothText.text = "Time Pump Right"
                lblDurationJust.text = "Duration Pump Right"
                lblDurationJustExpand.text = "Duration Pump Right"

            }
            else
            {
                lblTimeBothText.text = "Time Pump Both"
                lblDurationJust.text = "Duration Pump Both"
                lblDurationJustExpand.text = "Duration Pump Both"
    ...
    func checkItemChanged() {
        ...
        repeat {
           ...
           else  if logEvent.action.isEqualToString("both")
            {
                let timeLeftInit = formatter.stringFromDate(init_settingData.timeBoth!)
                let timeLeftSetting = formatter.stringFromDate(settingData.timeBoth!)
                if !timeLeftSetting.isEqual(timeLeftInit) {
                    bItemChanged = true
                    break
                }
        ...
        didSet {
            ...
            else
           {
            ...
            else  if settingData.optAction == PumpMoreSettingData.PumpActions.ActBoth
            {
                ...
                settingData.timeBoth = logEvent.timeBoth
                if settingData.timeBoth == nil
                {
                    settingData.timeBoth = NSDate.init()
                }
            ...
            {
                settingData.nLeftDurationHours = getIntValue(logEvent.bothHours, limitMin: 0, limitMax: 59, valueMin: 0, valueMax: 59)
                settingData.nLeftDurationMinutes = getIntValue(logEvent.bothMins, limitMin: 0, limitMax: 59, valueMin: 0, valueMax: 59)
            ...
            {
                settingData.nLeftDurationHours = getIntValue(logEvent.rightHours, limitMin: 0, limitMax: 59, valueMin: 0, valueMax: 59)
                settingData.nLeftDurationMinutes = getIntValue(logEvent.rightMins, limitMin: 0, limitMax: 59, valueMin: 0, valueMax: 59)
    ...
    func initBottleUIs() {
        ...
        if settingData.optAction == PumpMoreSettingData.PumpActions.ActRight
        {
            self.title = "Pump Right More"
        }
        else if settingData.optAction == PumpMoreSettingData.PumpActions.ActLeft        {
            self.title = "Pump Left More"
        }
        else
        {
            self.title = "Pump Both More"
        }
    ...
    override func tableView(tableView: UITableView, heightForRowAtIndexPath indexPath: NSIndexPath) -> CGFloat {
        ...
        let currCell:kPumpContentCell = kPumpContentCell(rawValue: indexPath.row)!
